...
async function simulationMain(): Promise<void> {
  ...
  type Stat = { pid: string; moves: number; elapsedMs: number; successes: number; failures: number };
  const perPlayer: Stat[] = [];

  const started = Date.now();

  const tasks: Array<Promise<void>> = [];
  for (let i = 0; i < PLAYERS; i++) tasks.push(playerTask(i));
  ...
  async function playerTask(index: number): Promise<void> {
    const pid = `p${index}`;
    let localSuccess = 0;
    let localFail = 0;
    let movesDone = 0;
    ...
    for (let move = 0; move < MOVES_PER_PLAYER; move++) {
      try {
        ...
        successFlips++; localSuccess++;
      } catch {
        failedFlips++; localFail++;
      }
      try {
        await timeout(randomDelay(MIN_DELAY_MS, MAX_DELAY_MS));
